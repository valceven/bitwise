...
const TeacherClassroom = ({ user }) => {
  ...
  const getClassroomColor = (id) => {
    const colors = ["bg-bluez", "bg-darkpurple", "bg-greenz", "bg-orangez", "bg-cyanz"];
    const textColors = ["text-bluez", "text-darkpurple", "text-greenz", "text-orangez", "text-cyanz"];
    const index = id?.length ? id.charCodeAt(0) % colors.length : Math.floor(Math.random() * colors.length);
    return {
      bg: colors[index],
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
            <div className="flex flex-wrap gap-4">
              ...
              <div className="flex space-x-1 rounded-lg bg-offwhite p-1">
                ...
                <button 
                  className={`px-4 py-2 rounded-lg ${activeTab === 'archived' ? 'bg-white shadow-sm' : 'hover:bg-gray-100'}`}
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-200 flex items-center">
            ...
            <div>
              <div className="text-2xl font-bold text-grayz">
                {
                  classrooms.length > 0 ? 
                  new Date(Math.max(...classrooms.map(c => new Date(c.createdAt)))).toLocaleDateString(undefined, {
                    month: "short",
                    day: "numeric"
                  }) : "-"
                }
        ...
        {isLoading ? (
                ...
                >
                  ...
                  <div className="p-6">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <h2 className="text-xl font-bold text-grayz line-clamp-1">{classroom.className}</h2>
                    ...
                    <div className="flex justify-between items-center pt-2 border-t border-gray-100">
                      <div className={`flex items-center ${colorScheme.text}`}>
                        <div className={`w-2 h-2 rounded-full ${colorScheme.bg} mr-1.5`}></div>
          ...
          <div className="text-center bg-white py-16 px-6 rounded-xl border border-gray-200 shadow-sm">
            ...
            <h3 className="text-xl font-bold text-grayz mb-2">No classrooms found</h3>
