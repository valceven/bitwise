...
import { ArrowLeft, Trophy, Target, Clock, Award, AlertTriangle, CheckCircle } from 'lucide-react';
...
const ASSESSMENT_CONFIG = {
  1: {
    ...
    description: "Explore the fascinating history of Boolean Algebra and its impact on modern computing"
  },
  2: {
    component: LogicGateExplorer,
    title: "Logic Gate Explorer",
    description: "Learn to identify and understand different types of logic gates"
  },
  3: {
    component: TruthTableConstructionAssessment,
    title: "Truth Table Construction",
    description: "Master the art of building truth tables for Boolean expressions"
  },
  4: {
  ...
  5: {
    ...
    description: "Speed challenge converting between Sum of Products and Product of Sums"
  },
  7: {
...
const useAssessmentData = (assessmentId) => {
  ...
  return { getLastScore, getBestScore, getAttempts, updateScore, resetAttempts, isCompleted };
};
...
const AssessmentStats = ({ stats, maxAttempts = 3, canRetry = true }) => {
  ...
  return stats.attempts > 0 ? (
    <div className={`border rounded-lg p-4 ${isMaxedOut ? 'bg-red-50 border-red-200' : 'bg-blue-50 border-blue-100'}`}>
        ...
        <div className="mt-3 p-3 bg-red-100 border border-red-200 rounded-lg">
          <div className="flex items-center">
            ...
            <span className="text-sm font-medium text-red-800">
              Maximum attempts reached. Contact your instructor to reset your attempts.
            </span>
          </div>
        </div>
      )}
...
const AssessmentNotFound = ({ assessmentId, onReturn }) => (
  <div className="bg-white rounded-lg shadow p-8 text-center">
    ...
    <p className="text-gray-600 mb-6">
      Assessment {assessmentId} could not be loaded. Please try again or select another.
    </p>
    <button onClick={onReturn} className="inline-flex items-center px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
...
const MaxAttemptsReached = ({ assessmentStats, onReturn, onReset }) => (
  <div className="bg-white rounded-lg shadow p-8 text-center">
    ...
    <h2 className="text-xl font-semibold text-gray-800 mb-2">Maximum Attempts Reached</h2>
...
const AssessmentView = () => {
  ...
  const currentAssessmentId = useMemo(() => assessmentId || topicId, [assessmentId, topicId]);
  ...
  const handleReturn = () => navigate(`/app/classroom/student/${classCode}`, { replace: true });
  ...
  const renderAssessment = () => {
    if (!config) {
      return <AssessmentNotFound assessmentId={currentAssessmentId} onReturn={handleReturn} />;
    }
          ...
          <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-6 mb-6">
            <div className="flex justify-between items-start">
              <button
                ...
                className="ml-6 inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700"
              >
                <ArrowLeft className="w-4 h-4 mr-2" />
                Back to Lessons
              </button>
