...
const LogicGate = ({
  ...
  const getConnectionPoints = () => {
    ...
    switch (type) {
      ...
      case "INPUT":
        ...
        break;
      case "OUTPUT":
        points.push({ id: "in1", type: "input", x: baseX + 10, y: baseY + 25 });
        break;
    }
    return points;
  };
  ...
  const gateSymbols = {
    AND: (
      <g>
        <path
          d="M0 0 L35 0 Q60 0 60 25 Q60 50 35 50 L0 50 Z"
          fill="white"
          stroke="black"
          strokeWidth="2"
        />
    ...
    OR: (
      <g>
        <path
          d="M0 0 Q35 0 45 25 Q35 50 0 50 Q20 25 0 0"
          fill="white"
          stroke="black"
          strokeWidth="2"
        />
...
const CircuitBuilderAcademy = ({
  ...
  const architectCases = [
    {
      ...
      scenario:
        "Design a circuit where the output is HIGH only when BOTH inputs A and B are HIGH.",
      ...
      points: 150,
      description:
        "Connect input A to the first input of an AND gate, input B to the second input, and the AND gate output to Y.",
    },

    {
      ...
      difficulty: "Apprentice",
      scenario:
        "Design a circuit where the output is HIGH when EITHER input A OR input B (or both) is HIGH.",
      ...
      points: 150,
      description:
        "Connect input A to the first input of an OR gate, input B to the second input, and the OR gate output to Y.",
    },

    {
      ...
      scenario:
        "Design a circuit that inverts the input signal - when A is HIGH, output should be LOW, and vice versa.",
      ...
      points: 180,
      description:
        "Connect input A to the input of a NOT gate, and the NOT gate output to Y.",
    },

    {
      ...
      difficulty: "Architect",
      scenario:
        "Design a circuit for: output HIGH when A is HIGH AND B is LOW.",
      ...
      points: 250,
      description:
        "Connect B to a NOT gate, then connect A and the NOT gate output to an AND gate, finally connect the AND output to Y.",
    },

    {
      ...
      difficulty: "Master",
      scenario:
        "Design a circuit for: output HIGH when (A OR B) AND the input C is LOW.",
      ...
      points: 350,
      description:
        "Use an OR gate for (A OR B), a NOT gate for (NOT C), then combine both outputs with an AND gate connected to Y.",
    },

    {
      ...
      difficulty: "Master",
      scenario:
        "Design a complex circuit: output HIGH when A is HIGH OR when both B and C are HIGH.",
      ...
      points: 400,
      description:
        "Connect B and C to an AND gate, then connect A and the AND gate output to an OR gate, with the OR output going to Y.",
    },
  ];

  // Calculate totals
  const totalQuestions = architectCases.length;
  const totalPossiblePoints = architectCases.reduce(
  ...
  const deleteGate = (gateId) => {
    ...
    setConnections(
      connections.filter(
        (conn) => conn.from.gateId !== gateId && conn.to.gateId !== gateId
      )
    );
  };
  ...
  const handleConnectionClick = (
    ...
    if (!isConnecting) {
      ...
      if (pointType === "input" && connectFrom) {
        // Validate connection
        if (
          connectFrom.gateId !== gateId &&
          connectFrom.pointType === "output"
        ) {
          // Check if this input is already connected
          const existingConnection = connections.find(
            (conn) => conn.to.gateId === gateId && conn.to.pointId === pointId
          );
  ...
  const evaluateUserCircuit = (inputValues) => {
    ...
    currentProblem.inputs.forEach((inputName) => {
      const inputGate = gates.find(
        (g) => g.id === inputName && g.type === "INPUT"
      );
      if (inputGate) {
  ...
  const testCircuit = () => {
    ...
    const answerData = {
      ...
      pointsEarned:
        isCorrect && !hasErrors
          ? Math.max(100, currentProblem.points - hintsUsed * 25)
          : 0,
    };
    ...
    return (
      ...
      >
        ...
        <div
          ...
          style={{
            background: `conic-gradient(${colors.indigoz} ${
              finalScore * 3.6
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 max-w-2xl mx-auto">
          ...
          <div
            className="p-4 rounded-xl"
            style={{ backgroundColor: `${colors.tealz}20` }}
          >
            <div className="text-2xl font-bold" style={{ color: colors.tealz }}>
              {accuracy}%
            </div>
            <div className="text-sm" style={{ color: colors.grayz }}>
              Success Rate
        ...
        >
          ...
          >
            {finalScore >= 80
              ? "Master Circuit Architect! 🏗️"
              : finalScore >= 60
              ? "Expert Digital Designer! ⚡"
              : finalScore >= 40
              ? "Good Circuit Builder! 👍"
              : "Apprentice Engineer! 📚"}
          </h3>
          <p className="text-sm" style={{ color: colors.grayz }}>
            {finalScore >= 80
              ? "Outstanding! You've mastered digital circuit design and Boolean logic implementation."
              : finalScore >= 60
              ? "Excellent work! You show strong skills in circuit building and logic gate connections."
              : finalScore >= 40
              ? "Well done! You're developing solid circuit design skills and Boolean understanding."
      ...
      <div className="space-y-8">
        ...
        >
          <div
            ...
            style={{
              background: `linear-gradient(135deg, ${colors.indigoz}, ${colors.tealz})`,
            }}
          >
        ...
        <motion.div
          ...
          style={{
            background: `linear-gradient(135deg, ${colors.white}, ${colors.indigoz}10)`,
          }}
        >
          <div
    ...
    return (
      ...
      >
        ...
        <p className="text-lg" style={{ color: colors.grayz }}>
          Perfect! Your circuit matches the Boolean expression. Moving to the
          next challenge...
        </p>
      </motion.div>
    );
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <div className="flex items-center gap-4">
          <div
            ...
            style={{
              backgroundColor:
                currentProblem.difficulty === "Apprentice"
                  ? `${colors.emeraldz}20`
                  : currentProblem.difficulty === "Architect"
                  ? `${colors.indigoz}20`
                  : `${colors.coralz}20`,
              color:
                currentProblem.difficulty === "Apprentice"
      ...
      >
        ...
        <div
          ...
          style={{ color: colors.grayz }}
        >
          💡 Click a gate type to add it to your circuit, then drag it to
          position
        </div>
      </motion.div>
      ...
      >
        ...
        >
              ...
              onDrag={(event, info) => {
                if (!gate.isFixed && !isConnecting) {
                  updateGatePosition(gate.id, {
                    x: Math.max(
                      0,
                      Math.min(700, gate.position.x + info.delta.x)
                    ),
                    y: Math.max(
                      0,
                      Math.min(320, gate.position.y + info.delta.y)
                    ),
                  });
                }
              }}
          ...
          {connections.map((connection, idx) => {
            ...
            const fromPos = getAbsoluteConnectionPosition(
              fromGate,
              connection.from.pointId
            );
            const toPos = getAbsoluteConnectionPosition(
              toGate,
        ...
        <div className="flex justify-between items-center mt-6 bg-gray-50 p-4 rounded-lg">
          ...
          {!showHint ? (
            <button
              ...
              style={{
                backgroundColor: `${colors.yellowz}30`,
                color: colors.ambez,
                border: `2px solid ${colors.yellowz}`,
              }}
            >
          ...
          >
            <div className="flex items-center gap-2 mb-2">
              <AlertTriangle
                ...
                style={{ color: colors.coralz }}
              />
              <span className="font-bold" style={{ color: colors.coralz }}>
                Circuit Issues:
              </span>
            </div>
            <ul className="text-sm space-y-1">
              {circuitErrors.map((error, idx) => (
                <li key={idx} style={{ color: colors.grayz }}>
                  • {error}
                </li>
              ))}
            </ul>
      ...
      >
        ...
        <div className="w-full max-w-md mx-auto h-2 bg-gray-200 rounded-full overflow-hidden">
          <motion.div
            ...
            initial={{ width: 0 }}
            animate={{
              width: `${((currentCase + 1) / architectCases.length) * 100}%`,
            }}
...
export default function CircuitBuilderAcademyMain({
  ...
  return (
    <div className="flex flex-col w-full max-w-7xl mx-auto pb-16 px-4 ">
      <div className="p-8">
